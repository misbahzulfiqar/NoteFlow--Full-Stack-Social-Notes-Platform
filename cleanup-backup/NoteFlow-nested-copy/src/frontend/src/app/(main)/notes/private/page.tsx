...
function PrivateNoteCard({ note }: { note: Note }) {
  return (
    <article className="overflow-hidden rounded-xl bg-white shadow-md ring-1 ring-slate-200/70">
      ...
      <div className="mt-3 flex items-center justify-between p-3 text-xs">
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Only your private notes are shown here
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto_auto]">
            <div className="flex items-center rounded-2xl bg-white px-4 py-2 shadow-sm ring-1 ring-slate-200">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && setSearch(searchInput.trim())
                }
        ...
        {isLoading ? (
          ...
          <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {notes.map((note) => (
              <PrivateNoteCard key={note.id} note={note} />
            ))}
          </section>
        )}
