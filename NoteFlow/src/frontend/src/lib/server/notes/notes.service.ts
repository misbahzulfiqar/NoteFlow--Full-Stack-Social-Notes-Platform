...
import type { CreateNoteInput, UpdateNoteInput } from "@/lib/server/notes/notes.validation";
...
export async function createNote(ownerId: string, input: CreateNoteInput): Promise<NoteResponse> {
...
export async function updateNote(
  ...
  const tags = input.tags !== undefined ? input.tags.map((t) => t.trim()) : ex.tags;
  const visibility = input.visibility !== undefined ? input.visibility : ex.visibility;
